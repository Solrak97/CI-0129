...
/* Ejemplo de soluci�n al problema 8 Puzzle utilizando el algoritmo A*. */
...
// se utiliza el caracter ' ' como el espacio vac�o,
// y los n�meros como su representaci�n ASCII (caracteres del '1' al '8 ).
using Tablero = std::array<std::array<char, NUM_COLUMNAS>, NUM_FILAS>;

// Almacena los tableros conocidos para no repetir nodos.
// Hay formas de mejorar la implementaci�n actual para eliminar esta tabla de b�squeda.
...
// Heredar e implementar estos m�todos es lo que tienen que hacer para resolver problemas con este c�digo.
class Nodo8Puzzle : public Nodo<Tablero>
{
public:
    Nodo8Puzzle(Tablero tablero) : Nodo<Tablero>(tablero) {}

    // Se implementa el m�todo abstracto que calcula las posibles transiciones o tableros vecinos
...
private:
    // Calcula si una fila y una columna pertenecen a la cuadr�cula del tablero.
    ...
    // Analiza los movimientos posibles en las direcciones v�lidas.
...
// este c�lculo representa qu� tan alejado est� esta ficha de su posici�n correcta en el tablero,
// o cu�ntos movimientos tengo que hacer para ponerla en su posici�n correcta.
...
// La heur�stica para un tablero es la suma de los costos de todas sus celdas.
...
int main()
{
    ...
    // Note que aqu� puede cambiar la heur�stica para observar diferentes resultados.
